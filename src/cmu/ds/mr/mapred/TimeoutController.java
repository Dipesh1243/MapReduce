...
public class TimeoutController implements Runnable{
	...
	public TimeoutController(Map<String, Set<TaskStatus>> tts,  TaskScheduler ts, Map<String, Integer> validtasktrackers){
			...
			synchronized(tasktrackers){
				for(String name: validtasktrackers.keySet()){
					if(validtasktrackers.get(name) > Util.TIME_OUT_MAX){
						...
						validtasktrackers.put(name, validtasktrackers.get(name) + 1);
					}
				}
